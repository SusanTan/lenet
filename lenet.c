...
void initialize_lenet(){
  lenet_C1 = initialize_4Dzeros(C1_COUT, C1_CIN,  CONV_SIZE, CONV_SIZE);
  kaiming_uniform(&lenet_C1, C1_CIN,  C1_COUT, CONV_SIZE);
  lenet_C3 = initialize_4Dzeros(C3_COUT, C1_COUT, CONV_SIZE, CONV_SIZE);
  kaiming_uniform(&lenet_C3, C1_COUT,  C3_COUT, CONV_SIZE);
  lenet_C5 = initialize_4Dzeros(C5_COUT, C3_COUT, CONV_SIZE, CONV_SIZE);
  kaiming_uniform(&lenet_C5, C3_COUT,  C5_COUT, CONV_SIZE);
  lenet_F6_W = initialize_zeroweights(F6_COUT, C5_COUT);
  uniform_W(&lenet_F6_W, C5_COUT, F6_COUT);
  lenet_OL_W = initialize_zeroweights(OL_COUT, F6_COUT);
  uniform_W(&lenet_OL_W, F6_COUT, OL_COUT);
  lenet_F6_B = initialize_zerobias(F6_COUT);
  uniform_B(&lenet_F6_B, C5_COUT, F6_COUT);
  lenet_OL_B = initialize_zerobias(OL_COUT);
  uniform_B(&lenet_OL_B, F6_COUT, OL_COUT);
  ...
  delta_OL_W = initialize_zeroweights(OL_COUT, F6_COUT);
  delta_F6_W = initialize_zeroweights(F6_COUT, C5_COUT);
  delta_OL_B = initialize_zerobias(OL_COUT);
  delta_F6_B = initialize_zerobias(F6_COUT);
  delta_C5   = initialize_4Dzeros(C5_COUT, C3_COUT, CONV_SIZE, CONV_SIZE);
  delta_C3   = initialize_4Dzeros(C3_COUT, C1_COUT, CONV_SIZE, CONV_SIZE);
  delta_C1   = initialize_4Dzeros(C1_COUT, C1_CIN , CONV_SIZE, CONV_SIZE);
}

void forward(int i, uint8_t mode)
{
  if(mode == 0) //train mode
    conv2d_forward(&lenet_C1, train_img_batch+i, C1_OUTSIZE, C1_CIN, C1_COUT, &out_C1);
  else
    conv2d_forward(&lenet_C1, test_img_batch+i, C1_OUTSIZE, C1_CIN, C1_COUT, &out_C1);
  TORCH_TANH(out_C1, C1_OUTSIZE, C1_COUT);
	maxpool2d_forward(POOL_STRIDE, POOL_SIZE, &out_C1, C1_COUT, S2_OUTSIZE, &out_S2, &S2_max_map);
  conv2d_forward(&lenet_C3, &out_S2, C3_OUTSIZE, C1_COUT, C3_COUT, &out_C3);
  TORCH_TANH(out_C3, C3_OUTSIZE, C3_COUT);
  maxpool2d_forward(POOL_STRIDE, POOL_SIZE, &out_C3, C3_COUT, S4_OUTSIZE, &out_S4, &S4_max_map);
  conv2d_forward(&lenet_C5, &out_S4, C5_OUTSIZE, C3_COUT, C5_COUT, &out_C5);
  TORCH_TANH(out_C5, C5_OUTSIZE, C5_COUT);
  linear_forward(&lenet_F6_W, &lenet_F6_B, &out_C5, C5_COUT, F6_COUT, &out_F6);
  TORCH_TANH(out_F6, F6_OUTSIZE, F6_COUT);
  linear_forward(&lenet_OL_W, &lenet_OL_B, &out_F6, F6_COUT, OL_COUT, &out_OL);
...
void backward(i)
{
  ...
  linear_backward  (&last_error, &out_F6, &lenet_OL_W,
                    F6_COUT, OL_COUT, &error_OL, &delta_OL_W, &delta_OL_B);
  linear_backward  (&error_OL, &out_C5, &lenet_F6_W,
                    C5_COUT, F6_COUT, &error_F6, &delta_F6_W, &delta_F6_B);
  conv_backward    (&error_F6, &out_S4, &lenet_C5, C3_COUT, C5_COUT,
                    &error_C5, CONV_SIZE, S4_OUTSIZE, C5_OUTSIZE, &delta_C5);
  pool_backward    (&error_C5, C3_COUT, &error_S4, POOL_STRIDE,
                    C3_OUTSIZE, &S4_max_map);
  conv_backward    (&error_S4, &out_S2, &lenet_C3, C1_COUT, C3_COUT,
                    &error_C3, CONV_SIZE, S2_OUTSIZE, C3_OUTSIZE, &delta_C3);
  pool_backward    (&error_C3, C1_COUT, &error_S2, POOL_STRIDE,
                    C1_OUTSIZE, &S2_max_map);
  conv_backward    (&error_S2, train_img_batch+i, &lenet_C1, C1_CIN, C1_COUT,
                    &error_C1, CONV_SIZE, C1_INSIZE, C1_OUTSIZE, &delta_C1);
}

void weight_update()
{
  //update, and also clear the buffer
  UPDATE_CONV(lenet_C1, delta_C1, C1_CIN,  C1_COUT, CONV_SIZE);
  UPDATE_CONV(lenet_C3, delta_C3, C1_COUT, C3_COUT, CONV_SIZE);
  UPDATE_CONV(lenet_C5, delta_C5, C3_COUT, C5_COUT, CONV_SIZE);
  UPDATE_LINEAR_W(lenet_F6_W, delta_F6_W, C5_COUT, F6_COUT);
  UPDATE_LINEAR_W(lenet_OL_W, delta_OL_W, F6_COUT, OL_COUT);
  UPDATE_LINEAR_B(lenet_F6_B, delta_F6_B, F6_COUT);
  UPDATE_LINEAR_B(lenet_OL_B, delta_OL_B, OL_COUT);
}
...
void free_all()
{
  free_conv(lenet_C1, C1_CIN,  C1_CIN,  CONV_SIZE);
  free_conv(lenet_C3, C1_COUT, C3_COUT, CONV_SIZE);
  free_conv(lenet_C5, C3_COUT, C5_COUT, CONV_SIZE);
  free_linear(lenet_F6_W, lenet_F6_B, F6_COUT);
  free_linear(lenet_OL_W, lenet_OL_B, OL_COUT);
  ...
  free_conv(delta_C5, C3_COUT, C5_COUT, CONV_SIZE);
  free_conv(delta_C3, C1_COUT, C3_COUT, CONV_SIZE);
  free_conv(delta_C1, C1_CIN,  C1_COUT, CONV_SIZE);
  free_linear(delta_F6_W, delta_F6_B, F6_COUT);
  free_linear(delta_OL_W, delta_OL_B, OL_COUT);
}
